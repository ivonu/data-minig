...
As a first implementation we tried clustering the users to user groups using a simple online k-Means algorithm with a $k=7$. Then we applied the simple plain UCB algorithm. For each user group we memorized the mean reward and the corresponding upper confidence bound. The performance of this algorithm however was quite poor.
